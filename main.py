...
def draw_hints(frame, is_gray_mode, width, height):
    ...
    y = 30
    hints = [
        ("Gray mode ON" if is_gray_mode else "Gray mode OFF", width - 250 if is_gray_mode else width - 200, y),
    ...
    for text, x, y_pos in hints:
        ...
        cv.rectangle(frame, (x - 2, y_pos - text_height - 2), (x + text_width + 2, y_pos + baseline + 2), color_bg, -1)
        # Draw the text itself
        cv.putText(frame, text, (x, y_pos), font, font_scale, color_text, thickness, cv.LINE_AA)
    return frame
...
def draw_detection(frame, x1, y1, x2, y2, conf):
    ...
    # Draw background rectangle for text
    cv.rectangle(frame, (label_x - 4, label_y - text_height - 4), (label_x + text_width + 4, label_y + baseline + 4), color_bg, -1)
    # Draw the text itself
    cv.putText(frame, label, (label_x, label_y), font, font_scale, color_text, thickness, cv.LINE_AA)
    return frame
...
def main():
    ...
    while True:
        ...
        if key == 27:  # Exit on ESC
            ...
            print("Switched to grayscale mode" if is_gray_mode else "Switched to color mode")
            ...
            video.release()
            video = cv.VideoCapture("v4l2src device=/dev/video0 ! videoconvert ! appsink", cv.CAP_GSTREAMER)
